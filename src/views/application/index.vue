    ...
    <div class="application">
      ...
      >
        <el-row>
          <el-col :span="12">
            <el-form-item label="项目名称">
              <div class="textLeft">
                <a :href="detail.announcementDocument.url">{{ name }}</a>
              </div>
            </el-form-item>
            ...
            <el-form-item label="营业执照扫描件">
              ...
              >
                <template v-if="imageUrl">
                  <img
                    v-if="imageUrl.includes('png') || imageUrl.includes('jpg')"
                    :src="imageUrl"
                    class="avatar"
                  />
                </template>
            ...
            <el-form-item label="授权委托人身份证扫描件">
              ...
              >
                <template v-if="imageUrl2">
                  <img
                    v-if="
                      imageUrl2.includes('png') || imageUrl2.includes('jpg')
                    "
                    :src="imageUrl2"
                    class="avatar"
                  />
            ...
            <el-form-item label="法定代表人身份证扫描件">
              ...
              >
                <template v-if="imageUrl3">
                  <img
                    v-if="
                      imageUrl3.includes('png') || imageUrl3.includes('jpg')
                    "
                    :src="imageUrl3"
                    class="avatar"
                  />
            ...
            <el-form-item label="授权委托书扫描件">
              ...
              >
                <template v-if="imageUrl4">
                  <img
                    v-if="
                      imageUrl4.includes('png') || imageUrl4.includes('jpg')
                    "
                    :src="imageUrl4"
                    class="avatar"
                  />
            ...
            <el-form-item label="付款凭证">
              ...
              >
                <template v-if="imageUrl5">
                  <img
                    v-if="
                      imageUrl5.includes('png') || imageUrl5.includes('jpg')
                    "
                    :src="imageUrl5"
                    class="avatar"
                  />
        ...
        <el-dialog title="报名提示" :visible.sync="dialogVisible" width="60%">
          <span class="password"
            >投标密码：<span style="font-family: cursive">{{
              password
            }}</span></span
          >
          <span class="password"
          ...
          >
          <span class="content"
            >招标文件：<a :href="detail.biddingDocument.url">{{
              detail.biddingDocument.name
            }}</a></span
          >
  ...
  methods: {
    ...
    onSubmit2() {
      this.$refs.form.validate((valid) => {
        if (valid) {
          ...
          this.$message.warning(`有内容未填写，请审核！`);
          return false;
        }
      });
    },
    ...
    postApplication() {
      ...
      postApplication({
        ...
        detail: {
          ...this.form,
          businessLicenses: [this.$refs.upload.uploadFiles[0].response.data],
          consignors: [this.$refs.upload2.uploadFiles[0].response.data],
          corporates: [this.$refs.upload3.uploadFiles[0].response.data],
          authorizations: [this.$refs.upload4.uploadFiles[0].response.data],
          evidencePayments: [this.$refs.upload5.uploadFiles[0].response.data],
        },
      }).then((res) => {
        if (res.code == 0) {
          this.lock = true;
    ...
    handleExceed(files, fileList) {
      this.$message.warning(`当前限制上传1个文件，请重新上传`);
    },
  },
};
</script>
